...
public class ConsoleUI implements Runnable {
	...
	private Node findNextNode(Node node) {
		if (node == null)
			return null;
		if (node.isEnabled() && !node.isSatisfied())
			return node;
		if (node.getSubtree() != null)
			return findNextNode(node.getSubtree());
	...
	public static void start(URL config, InputStream in, OutputStream out)
		...
		ConsoleUI ui = new ConsoleUI(new BufferedReader(new InputStreamReader(
				in)), new PrintWriter(new OutputStreamWriter(out)),
				configurator);
